...
def run(state, config):
    ...
    plan_key = kv_store.mk_dirspace_key(state, 'plan.text')
    config['tee'] = os.path.join(state.outputs_dir, plan_key)
    ...
    diff_key = kv_store.mk_dirspace_key(state, 'diff')
    config['tee'] = os.path.join(state.outputs_dir, diff_key)
    ...
    diff_json_key = kv_store.mk_dirspace_key(state, 'diff_json')
    config['tee'] = os.path.join(state.outputs_dir, diff_json_key)
    ...
    return workflow.Result2(
        payload={
            ...
            '@diff': os.path.basename(diff_json),
            'has_changes': has_changes,
            '@text': os.path.basename(plan_stdout_key),
            '@stderr': os.path.basename(plan_stderr_key),
            'visible_on': 'always',
